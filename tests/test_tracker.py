...
def test_should_mark_dead_true(tracker_scraper):
    ...
    assert tracker_scraper._should_mark_dead(infohash) == True
...
def test_should_mark_dead_false_not_enough_stats(tracker_scraper):
    ...
    assert tracker_scraper._should_mark_dead(infohash) == False
...
def test_update_stats_marks_dead(tracker_scraper):
    ...
    with patch('nyaastats.tracker.datetime') as mock_datetime:
        ...
        with tracker_scraper.db.get_conn() as conn:
            cursor = conn.execute("SELECT status FROM torrents WHERE infohash = ?", (infohash,))
            row = cursor.fetchone()
...
def test_update_batch_stats(tracker_scraper):
    ...
    with patch.object(tracker_scraper, 'update_stats') as mock_update:
        ...
        mock_update.assert_any_call('abcdef1234567890abcdef1234567890abcdef12', {
            'seeders': 10,
            'leechers': 2,
            'downloads': 100
        })
        mock_update.assert_any_call('fedcba0987654321fedcba0987654321fedcba09', {
            'seeders': 5,
            'leechers': 1,
            'downloads': 50
        })

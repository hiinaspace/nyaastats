    ...
    with patch('nyaastats.rss_fetcher.httpx.Client') as mock_client:
        ...
        with patch('nyaastats.rss_fetcher.guessit.guessit') as mock_guessit:
            ...
            with temp_db.get_conn() as conn:
                cursor = conn.execute("SELECT * FROM torrents WHERE infohash = ?", 
                ...
                assert row['trusted'] == False
                assert row['remake'] == False
                ...
                cursor = conn.execute("SELECT * FROM stats WHERE infohash = ?", 
                                    ('f87db04e1531c5f6fbaca3e6e2876f9c2982f46a',))
...
def test_tracker_integration(temp_db, example_rss_content):
    ...
    with patch.object(tracker_scraper, 'scrape_batch') as mock_scrape:
        ...
        stats1 = temp_db.get_recent_stats('f87db04e1531c5f6fbaca3e6e2876f9c2982f46a', limit=1)
        stats2 = temp_db.get_recent_stats('20a760df29d3bea030cf5f920ae5c932ca78f1b3', limit=1)
...
def test_dead_torrent_detection(temp_db):
    ...
    with temp_db.get_conn() as conn:
        cursor = conn.execute("SELECT status FROM torrents WHERE infohash = ?", 
...
<rss version="2.0" xmlns:nyaa="https://nyaa.si/xmlns/nyaa">
    ...
    with patch('nyaastats.rss_fetcher.httpx.Client') as mock_client:
        ...
        with patch('nyaastats.rss_fetcher.guessit.guessit') as mock_guessit:
            ...
            assert temp_db.get_torrent_exists('failedbeef1234567890failedbeef1234567890')

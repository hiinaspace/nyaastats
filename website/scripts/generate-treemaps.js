...
const outputDir = path.join(dataDir, "treemaps");
...
function formatDateStacked(weekStr, startDate) {
  ...
  const endFmt = end.toLocaleDateString("en-US", { ...utc, month: "short", day: "numeric" });
...
async function renderTreemapSvg(weeks, weekData, weekIndex) {
  ...
  const root = hierarchy({
    name: "root",
    children: weekData.rankings.map((r) => ({ ...r, value: r.downloads }))
  })
    .sum((d) => d.value)
  ...
  for (let i = 0; i < leaves.length; i++) {
    ...
    svgParts.push(`<rect x="${x0}" y="${y0}" width="${cellWidth}" height="${cellHeight}" fill="${leaf.data.cover_image_color || "#1a1a2e"}" stroke="#222" stroke-width="1"/>`);

    const dataUri = await fetchImageDataUri(leaf.data.cover_image_url);
    if (dataUri) {
      svgParts.push(`<image href="${dataUri}" x="${x0}" y="${y0}" width="${cellWidth}" height="${cellHeight}" preserveAspectRatio="xMidYMid slice" clip-path="url(#${clipId})"/>`);
    }

    const textX = x0 + 6 * scale;
    if (leaf.data.rank <= 40) {
      ...
      if (!isOldestWeek) {
        if (isNew) {
          ...
          rankDeltaText = ` UP ${rankChange}`;
          rankDeltaColor = "#4ade80";
        } else if (rankChange < 0) {
          rankDeltaText = ` DOWN ${Math.abs(rankChange)}`;
      ...
      svgParts.push(`<rect x="${x0}" y="${overlayY}" width="${cellWidth}" height="${overlayHeight}" fill="url(#${gradientId})"/>`);
      svgParts.push(`<text x="${textX}" y="${rankY}" font-size="${13 * scale}" font-weight="700" fill="#fff" stroke="${textStroke}" stroke-width="${textShadowWidth}" paint-order="stroke" font-family="Arial, sans-serif" clip-path="url(#${clipId})">#${leaf.data.rank}${rankDeltaText ? `<tspan fill="${rankDeltaColor}">${rankDeltaText}</tspan>` : ""}</text>`);
      svgParts.push(`<text x="${textX}" y="${titleY}" font-size="${11 * scale}" font-weight="600" fill="#fff" stroke="${textStroke}" stroke-width="${textShadowWidth}" paint-order="stroke" font-family="Arial, sans-serif" clip-path="url(#${clipId})">${title}</text>`);
      svgParts.push(
        `<text x="${textX}" y="${bottomY}" font-size="${10 * scale}" fill="#ddd" stroke="${textStroke}" stroke-width="${textShadowWidth}" paint-order="stroke" font-family="Arial, sans-serif" clip-path="url(#${clipId})">` +
          `${downloads}${downloadChangePct == null || downloadChangePct === 0 ? "" : `<tspan fill="${dlDeltaColor}"> ${downloadChangePct > 0 ? "+" : ""}${downloadChangePct}%</tspan>`}` +
          "</text>"
      );
    } else {
      svgParts.push(`<text x="${textX}" y="${y0 + 14 * scale}" font-size="${10 * scale}" font-weight="700" fill="#fff" stroke="${textStroke}" stroke-width="${textShadowWidth}" paint-order="stroke" font-family="Arial, sans-serif" clip-path="url(#${clipId})">#${leaf.data.rank}</text>`);
    }
  }
  ...
  const subtitleHeight = subtitleLines.length * subtitleLineHeight + subtitleGap;

  for (let i = 0; i < subtitleLines.length; i++) {
    const line = escapeXml(subtitleLines[subtitleLines.length - 1 - i]);
    const y = height - margin - i * subtitleLineHeight;
    svgParts.push(`<text x="${width - margin}" y="${y}" font-size="${subtitleFontSize}" font-weight="600" fill="${watermarkFill}" stroke="${watermarkStroke}" stroke-width="${6 * scale}" paint-order="stroke" text-anchor="end" font-family="Arial, sans-serif">${line}</text>`);
  }

  for (let i = 0; i < titleLines.length; i++) {
    const line = escapeXml(titleLines[titleLines.length - 1 - i]);
    const y = height - margin - subtitleHeight - i * titleLineHeight;
    svgParts.push(`<text x="${width - margin}" y="${y}" font-size="${titleFontSize}" font-weight="700" fill="${watermarkFill}" stroke="${watermarkStroke}" stroke-width="${10 * scale}" paint-order="stroke" text-anchor="end" font-family="Arial, sans-serif">${line}</text>`);
  }
...
async function main() {
  ...
  for (let i = 0; i < weeks.length; i++) {
    const week = weeks[i];
    const outputPath = path.join(outputDir, `treemap-${week.week}.jpg`);
    const svg = await renderTreemapSvg(weeks, week, i);
    const jpgBuffer = await sharp(Buffer.from(svg)).jpeg({ quality: 82 }).toBuffer();
    fs.writeFileSync(outputPath, jpgBuffer);
    console.log(`treemap: wrote ${path.relative(rootDir, outputPath)}`);
  }
}

main().catch((error) => {
  console.error(`treemap: failed to render images: ${error.stack || error}`);
  process.exit(1);
});

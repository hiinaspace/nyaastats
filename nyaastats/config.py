...
class Settings(BaseSettings):
    ...
    # Database
    db_path: str = Field(default="nyaastats.db", description="Path to SQLite database file")
    ...
    rss_fetch_interval_hours: int = Field(
        default=1,
        description="Interval between RSS fetches in hours"
    )
    ...
    scrape_batch_size: int = Field(
        default=40,
        description="Number of torrents to scrape in one batch"
    )
    scrape_interval_seconds: int = Field(
        default=60,
        description="Interval between scrape operations in seconds"
    )

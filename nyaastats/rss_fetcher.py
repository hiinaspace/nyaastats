...
class RSSFetcher:
    def __init__(self, db: Database, feed_url: str = "https://nyaa.si/?page=rss&c=1_2&f=0"):
        self.db = db
        self.feed_url = feed_url
        self.client = httpx.Client(timeout=30.0)

    def fetch_feed(self, page: Optional[int] = None) -> feedparser.FeedParserDict:
    ...
    def parse_entry(self, entry: feedparser.FeedParserDict) -> Tuple[Dict[str, Any], Dict[str, Any]]:
            ...
            try:
                ...
                for key, value in guessit_data.items():
                    if hasattr(value, '__fspath__'):
                        ...
                        guessit_data[key] = [str(item) if hasattr(item, '__fspath__') else item for item in value]
            except Exception as e:
    ...
    def process_feed(self, page: Optional[int] = None) -> int:
        ...
        for entry in feed.entries:
            try:
                ...
                if not torrent_data['infohash'] or not torrent_data['filename']:
                    logger.warning(f"Skipping entry with missing infohash or filename: {entry.get('title', 'Unknown')}")
                    continue
